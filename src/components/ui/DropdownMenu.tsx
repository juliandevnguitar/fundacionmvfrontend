...
export default function DropdownMenu({locale, translations} : {locale: string, translations: translations}){
    ...
    return (
        <div className={cn({dropdown: true, 'dropdown-open': open, 'dropdown-end ml-4': true})}>
            ...
            {/* hamburger icon */}
            <svg className={cn({'swap-off current-fill h-8 w-8 text-primary': open, })} xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 512 512"><path d="M64,384H448V341.33H64Zm0-106.67H448V234.67H64ZM64,128v42.67H448V128Z"/></svg>
            
            {/* close icon */}
            <svg className={cn({'swap-on current-fill h-8 w-8 text-primary' : !open, 'text-primary': true})} xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 512 512"><polygon points="400 145.49 366.51 112 256 222.51 145.49 112 112 145.49 222.51 256 112 366.51 145.49 400 256 289.49 366.51 400 400 366.51 289.49 256 400 145.49"/></svg>
            
            </label>
            
            <ul tabIndex={0} className={cn({
                'menu menu-lg dropdown-content mt-3 items-center justify-between z-[20] p-2 shadow bg-base-100 rounded-box  h-[30vh]': true,
                hidden: !open
            })}>
